...
function sortTRLByAuthor(user){
  ...
  if(xhr!=null){
	if (AuthorAscending){
		...
		AuthorAscending = 0;
		//alert("scripts/db/getBookList.php?u="+user+"&sort=Title&sortOrder=DESC");
		xhr.open("GET","scripts/db/getFriendTRLBookList.php?u="+user+"&sort=Author&sortOrder=ASC", true);
		xhr.onreadystatechange = function(){
			if ( xhr.readyState == 4 ){
				bookList.innerHTML = xhr.responseText;
				filter();
		...
		//populate bookList with the AJAX request results
		//bookList.innerHTML = xhr.responseText;
		xhr.open("GET","scripts/db/getFriendTRLBookList.php?u="+user+"&sort=Author&sortOrder=DESC", true);
		xhr.onreadystatechange = function(){
			if ( xhr.readyState == 4 ){
				bookList.innerHTML = xhr.responseText;
				filter();
...
function sortTRLByTitle(user){
  ...
  if (TitleAscending){
	//make an ajax request to get the result ORDER BY AuthorName ASC, AuthorForname ASC.
	TitleAscending = 0;
	xhr.open("GET","scripts/db/getFriendTRLBookList.php?u="+user+"&sort=Title&sortOrder=ASC", true);
	xhr.onreadystatechange = function(){
		if ( xhr.readyState == 4 ){
			bookList.innerHTML = xhr.responseText;
			filter();
	...
	xhr.open("GET","scripts/db/getFriendTRLBookList.php?u="+user+"&sort=Title&sortOrder=DESC", true);
	xhr.onreadystatechange = function(){
		if ( xhr.readyState == 4 ){
			bookList.innerHTML = xhr.responseText;
			filter();
